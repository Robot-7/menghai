...
public class EntityManagerImpl implements EntityManager{
	...
	protected void initHibernate() {
		if(this.conf == null) {
			String fileName = DZM.basePath + "/hibernateconfig/hibernate_" + DZM.platForm + "_" + DZM.serverId + ".cfg.xml";
			File file = new File(fileName);
			this.conf = new Configuration();
			this.conf.configure(file);
		}
	 ...
	 */
	public <T> List<T> limitQuery(Class<T> klass, String hql, int start,
...
//		}
	 ...
	 */
	public synchronized <T> List<T> limitQuery(String hql, int start,
			int count, Object... values) throws DataAccessException{
		try{
			Session session = getSession();
			Transaction tx = session.beginTransaction();
