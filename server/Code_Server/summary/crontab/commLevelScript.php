...
while($total = mysql_fetch_array($resultserver)){
	...
	$commLevelTotal = mysql_query("select userid from stat_player_info where serverid = $serverid");
	$num_rows = mysql_num_rows($commLevelTotal);
	
	$commLevel = mysql_query("select count(1),userlevel from stat_player_info where serverid = $serverid group by userlevel ");

	while($total = mysql_fetch_array($commLevel)){
		$queryinsert = sprintf("insert into stat_common_level (serverid,date,level,num,perCent) values ('%d','%s','%d','%d','%f')",$serverid,date('Ymd',strtotime("-$needday day")),$total[1],$total[0],$total[0]/$num_rows);
