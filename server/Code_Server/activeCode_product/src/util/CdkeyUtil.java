...
public class CdkeyUtil {
	//获取cdkey
	public static String createCdkey(String type, String channel) { // length表示生成字符串的长度
	...
	//批量获取cdkey
	public static List<ActiveCodeEntity> createCdkeysList(int count, String type, String channel) {
		List<String> list = new ArrayList<String>();
		List<ActiveCodeEntity> cdkeys=new ArrayList<ActiveCodeEntity>();
		for (int i = 0; i < count; i++) {
			...
			{
				ActiveCodeEntity cdkey=new ActiveCodeEntity();
				cdkey.setActive_code(cd_key);
	...
	public static String createCondition(String type,String channel, String status){
		...
		if(StringUtils.isNotBlank(type)){
			condition+=" and ac.active_code like '"+type+"%' ";
		}
		if(StringUtils.isNotBlank(channel)){
			condition+=" and ac.active_code like '%"+channel+"' ";
		}
		if(StringUtils.isNotBlank(status)){
			condition+=" and ac.status="+status+" ";
		}
		return condition;
	}
