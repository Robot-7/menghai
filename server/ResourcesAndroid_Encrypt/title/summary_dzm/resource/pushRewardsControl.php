...
function pay_result_notify_process($MyAppId,$MyAppKey){
	...
	require_once('../proto/pb_proto_BackStagePushRewardInfo.php');
	...
	if(empty($_GET)||!isset($_GET['field'])||!isset($_GET['rewardname'])||!isset($_GET['rewardcontent'])||!isset($_GET['logintime'])||!isset($_GET['names'])||!isset($_GET['levelStart'])||!isset($_GET['levelEnd'])||!isset($_GET['rankStart'])||!isset($_GET['rankEnd'])){
	...
	$rewardcontent = $_GET['rewardcontent'];//用户ID
	$logintime = $_GET['logintime'];
	$names = $_GET['names'];
	$levelStart = $_GET['levelStart'];
	$levelEnd = $_GET['levelEnd'];
	$rankStart = $_GET['rankStart'];
	$rankEnd = $_GET['rankEnd'];
	...
	$OPBackStagePushRewardInfo = new OPBackStagePushRewardInfo();
	$OPBackStagePushRewardInfo->set_version(1);
	$OPBackStagePushRewardInfo->set_field($field);
	$OPBackStagePushRewardInfo->set_rewardname($rewardname);
	$OPBackStagePushRewardInfo->set_rewardcontent($rewardcontent);
	$OPBackStagePushRewardInfo->set_logintime($logintime);
	$OPBackStagePushRewardInfo->set_names($names);
	$OPBackStagePushRewardInfo->set_levelStart($levelStart);
	$OPBackStagePushRewardInfo->set_levelEnd($levelEnd);
	$OPBackStagePushRewardInfo->set_rankStart($rankStart);
	$OPBackStagePushRewardInfo->set_rankEnd($rankEnd);
	
	$data = $OPBackStagePushRewardInfo->SerializeToString();
	...
	$OPBackStagePushRewardInfoRet = new OPBackStagePushRewardInfoRet();
	$OPBackStagePushRewardInfoRet->parseFromString(hexDecode($out));
	$Result['servertime'] = $OPBackStagePushRewardInfoRet->servertime();
