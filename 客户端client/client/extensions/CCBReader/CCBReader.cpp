...
CCNode* CCBReader::readNodeGraphFromFile(const char *pCCBFileName, CCObject *pOwner, const CCSize &parentSize)
{
	...
	static CCDictionary CCBIFileDic;
	CCData *data = dynamic_cast<CCData *>(CCBIFileDic.objectForKey(strPath.c_str()));
	if(!data)
	{
		unsigned char * pBytes = CCFileUtils::sharedFileUtils()->getFileData(strPath.c_str(), "rb", &size);
		data = new CCData(pBytes, size);
		CC_SAFE_DELETE_ARRAY(pBytes);
		CCBIFileDic.setObject(data,strPath.c_str());
	}
...
float CCBReader::getResolutionScale()
{
	...
	return cocos2d::CCDirector::sharedDirector()->getContentScaleFactor();
}
