	    ...
	    case kCCBPropTypeFloatXY:
	      {
            ...
            case kCCBPropTypeTexture: 
            {
				...
				if(setProp)
				{
					CCSpriteFrame * ccSpriteFrame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(spriteFile.c_str());
					if(!ccSpriteFrame)
					{
						CCTexture2D * ccTexture2D = CCTextureCache::sharedTextureCache()->addImage(spriteFile.c_str());
						if(ccTexture2D)
						{
...
CCSpriteFrame * CCNodeLoader::parsePropTypeSpriteFrame(CCNode * pNode, CCNode * pParent, CCBReader * pCCBReader, const char *pPropertyName) 
{
    ...
    if (spriteFile.length() != 0)
    {
        if (spriteSheet.length() == 0)
        {
			...
			spriteFrame = CCSpriteFrameCache::sharedSpriteFrameCache()->spriteFrameByName(spriteFile.c_str());
			if(!spriteFrame)
			{
            ...
            if(target != NULL) {
                if(selectorName.length() > 0) {
                    ...
                    if(targetAsCCBSelectorResolver != NULL) {
						...
						if(!selMenuHandler)
							selMenuHandler = targetAsCCBSelectorResolver->onResolveCCBCCMenuItemSelectorWithSender(target, selectorName.c_str(),pNode);
					}
                    if(selMenuHandler == 0) {
                        ...
                        if(ccbSelectorResolver != NULL) {
							...
							if(!selMenuHandler)
								selMenuHandler = ccbSelectorResolver->onResolveCCBCCMenuItemSelectorWithSender(target, selectorName.c_str(),pNode);
						}
                    }
