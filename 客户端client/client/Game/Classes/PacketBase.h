...
private:
	...
	/**should delete buffer after use*/
	bool UnpackPacket(void*buffer , int length);
	static std::string UnpackPacket(int opcode, void *buffer, int _length);
};
...
public:virtual PacketBase* createPacket(){return new PacketBase(new PROTOBUF_MESSAGE,OPCODE);}}; \
	...
	static bool __ret_for_##PROTOBUF_MESSAGE = PacketManager::Get()->_regisiterPacketFactory(OPCODE, #PROTOBUF_MESSAGE,__factory_for_##PROTOBUF_MESSAGE);
