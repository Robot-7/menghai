...
void Popup1stPayTipPage::_refreshPage()
{
	...
	if(mType==GoldNotEnough)
	{
		...
		CCB_FUNC(this,"mTitle",CCLabelBMFont,setString(Language::Get()->getString("@BUYNotEnoughRMB").c_str()));
	}
	else if(mType==SilverNotEnough)
	{
		strList.push_back(Language::Get()->getString("@BeiLi"));
		CCB_FUNC(this,"mTitle",CCLabelBMFont,setString(Language::Get()->getString("@BUYNotEnoughBL").c_str()));
	}
	...
	CCB_FUNC(this,"mPopMsg",CCLabelBMFont,setString(outstr.c_str()));

	CCB_FUNC(this,"mFunction1",CCLabelBMFont,setString(Language::Get()->getString("@Close").c_str()));

	CCB_FUNC(this,"mFunction",CCLabelBMFont,setString(Language::Get()->getString("@Recharge").c_str()));
