...
MESSAGE_FACTORY_BEGIN(MsgContinueLoginFlipCard)
	int index;
sscanf(paras.c_str(), "%d",&index);
ret->index = index;
MESSAGE_FACTORY_END(MsgContinueLoginFlipCard,MSG_CONTINUE_LOGIN_FLIPCARD);
...
MESSAGE_FACTORY_BEGIN(MsgAdventureTeachExpDataInit)
	int index;
sscanf(paras.c_str(), "%d",&index);
ret->index = index;
MESSAGE_FACTORY_END(MsgAdventureTeachExpDataInit,MSG_ADVENTURE_TEACHEXPINIT);



MESSAGE_FACTORY_BEGIN(MsgPackPreviewSourceMsg)
	int index;
sscanf(paras.c_str(), "%d",&index);
ret->index = index;
MESSAGE_FACTORY_END(MsgPackPreviewSourceMsg,MSG_PACKPREVIEW_SOURCE_MSG);
