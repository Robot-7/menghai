...
package com.youai.dreamonepiece.platform.nd91;
...
public class Platform91LoginAndPay implements IPlatformLoginAndPay {
	...
	public void init(IGameActivity game_acitivity, GameInfo game_info) {
		...
		//game_info.app_id = 101409;
		//game_info.app_key = "6acdc9a1c9a64a6871c19dd4dd7fcb185fc05c4c6a240b56";
		...
		//
		NdCommplatform.getInstance().ndSetScreenOrientation(this.game_info.screen_orientation);
		//
		...
		NdCommplatform.getInstance().ndInit(game_ctx, appInfo, new OnInitCompleteListener()
		{
				...
				new OnSwitchAccountListener() { //设置玩家在社区界面�?择注�?��号时的监听事�?
					...
					public void onSwitchAccount(int code) {
						if (code == NdErrorCode.ND_COM_PLATFORM_ERROR_USER_SWITCH_ACCOUNT) {
							...
							login_info.login_session = NdCommplatform.getInstance().getSessionId();
							login_info.account_uid_str = NdCommplatform.getInstance().getLoginUin();
							login_info.account_uid = Long.parseLong(login_info.account_uid_str);
							login_info.account_nick_name = NdCommplatform.getInstance().getLoginNickName();
							//
							Platform91LoginAndPay.getInstance().notifyLoginResult(login_info);
							//
							Toast.makeText(
									Platform91LoginAndPay.getInstance().game_ctx,
									"新帐号登录成功", Toast.LENGTH_SHORT)
									.show();
						} else if (code == NdErrorCode.ND_COM_PLATFORM_ERROR_CANCEL) {
							//玩家点击社区注销账号按钮但未输入新账号登录，而是�?��界面可捕捉此状�?
							Toast.makeText(
									Platform91LoginAndPay.getInstance().game_ctx,
									"取消帐号登录", Toast.LENGTH_SHORT)
									.show();
						} else {
							// 玩家点击社区注销账号按钮输入新的账号并登录失败时可捕捉此状�?
							Toast.makeText(
									Platform91LoginAndPay.getInstance().game_ctx,
									"帐号登录失败", Toast.LENGTH_SHORT)
									.show();
						}
					}
				});
		...
		NdCommplatform.getInstance().setOnSessionInvalidListener(
				new OnSessionInvalidListener() {
					...
					public void onSessionInvalid() {
						...
						Toast.makeText(
								Platform91LoginAndPay.getInstance().game_ctx,
								"会话失效，将重新登录", Toast.LENGTH_SHORT)
								.show();
					}
				});
		
		try {
			 ApplicationInfo info = Platform91LoginAndPay.getInstance().game_ctx.getPackageManager().getApplicationInfo(Platform91LoginAndPay.getInstance().game_ctx.getPackageName(),PackageManager.GET_META_DATA);
			TalkingDataGA.init(Platform91LoginAndPay.getInstance().game_ctx, info.metaData.getString("TDGA_APP_ID"), info.metaData.getString("TDGA_CHANNEL_ID"));
	...
	public void callLogin() {
		...
		NdCommplatform.getInstance().ndLogin(this.game_ctx, new NdMiscCallbackListener.OnLoginProcessListener() {
			...
			public void finishLoginProcess(int code) {
				...
				Log.d(Platform91LoginAndPay.TAG, 
						"NdCommplatform.getInstance().ndLogin, responseCode: " + String.valueOf(code));
				if (code == NdErrorCode.ND_COM_PLATFORM_SUCCESS) {
					...
					//
					Platform91LoginAndPay.getInstance().notifyLoginResult(login_info);
					TDGAAccount.setAccount(TalkingDataGA .getDeviceId(Platform91LoginAndPay.getInstance().game_ctx));
					//
				} else if (code == NdErrorCode.ND_COM_PLATFORM_ERROR_CANCEL) {
					tip = "取消登录";
					...
					if (Platform91LoginAndPay.getInstance().auto_recalllogin_count < 5) {
						...
						//if (Platform91LoginAndPay.getInstance().auto_recalllogin_count >= 2)
							Platform91LoginAndPay.getInstance().callLogin();
					}
					else {
				...
				Toast.makeText(Platform91LoginAndPay.getInstance().game_ctx, tip, Toast.LENGTH_SHORT).show(); 
			} 
		}); 
	...
	public void notifyLoginResult(LoginInfo login_result) {
		...
		if (login_result != null) {
			...
			login_info.account_uid_str = PlatformAndGameInfo.enPlatformShort_91+login_info.account_uid_str;
	...
	public void callLogout() {
		...
		NdCommplatform.getInstance().ndLogout(NdCommplatform.LOGOUT_TO_NON_RESET_AUTO_LOGIN_CONFIG, game_ctx);
	}
	...
	public void callCheckVersionUpate() {
		...
		NdCallbackListener<Integer> callback = new NdCallbackListener<Integer>() {
			...
			public void callback(int responseCode, Integer t) {
				Log.d(Platform91LoginAndPay.TAG, 
						"callCheckVersionUpate, responseCode: " + String.valueOf(responseCode) 
				...
				if (responseCode == NdErrorCode.ND_COM_PLATFORM_SUCCESS) {
					...
					case NdCommplatform.UPDATESTATUS_NONE:
					{
						...
						Platform91LoginAndPay.getInstance().notifyVersionUpateInfo(version_info);
						break;
					}
					...
					/*	VersionInfo version_info = new VersionInfo();
						version_info.update_info = PlatformAndGameInfo.enUpdateInfo_Suggest;
						Platform91LoginAndPay.getInstance().notifyVersionUpateInfo(version_info);*/
						AlertDialog dlg = new AlertDialog.Builder(Platform91LoginAndPay.getInstance().game_ctx)
						.setTitle("提示")
						.setMessage("游戏已经开始下载，请在手机消息通知栏查看下载状态。（从屏幕最上方往下滑打开状态栏）")
					...
					Toast.makeText(Platform91LoginAndPay.getInstance().game_ctx, "91游戏更新失败", Toast.LENGTH_LONG).show();
				}
				//
			}
		...
		// 91SDK�?��游戏版本更新
		NdCommplatform.getInstance().ndAppVersionUpdate(this.game_ctx, callback);
		// 另见SDK有不带提示及下载功能的ndCheckVe rsionUpdate使用方法
	}
				...
				game_ctx, new NdMiscCallbackListener.OnPayProcessListener() {
					...
					public void finishPayProcess(int code) {
						...
						Platform91LoginAndPay.getInstance()
							.notifyPayRechargeRequestResult(Platform91LoginAndPay.getInstance().pay_info);
					}
				});
		//
		if (aError != 0) {
	...
	public void onGameExit()
	{
		...
		NdCommplatform.getInstance().ndExit(new OnExitCompleteListener(this.game_ctx) {
	...
	public void onGameResume() {
		...
		NdCommplatform.getInstance().ndPause(new OnPauseCompleteListener(this.game_ctx) {
