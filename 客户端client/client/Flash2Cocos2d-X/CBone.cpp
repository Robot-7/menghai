...
void CCBone::update( float dt ) {
	...
	mTransformX = mLockX + mTweenNode->x;
	mTransformY = mLockY + mTweenNode->y;
	...
	if (mParent) {
		...
		float _dX = mLockX + mTweenNode->x;
		float _dY = mLockY + mTweenNode->y;
		...
		//CCXLOG("%s, %f, %f, %f, %f", this->mName.c_str(), mTransformX , mParentX, mTransformY , mParentY);
	}
...
void CCBone::updateDisplay(){
	...
	if (mDisplay) {
		...
		if (mIsRadian) {
			mDisplay->setRotation( (mTweenNode->rotation + mParentR + mLockR) * ANGLE_TO_RADIAN );
		}else {
			mDisplay->setRotation( mTweenNode->rotation + mParentR + mLockR );
		}
